...
- Un intento posterior de `POST /api/billing/subscribe` llegó a Mercado Pago y recibió HTTP 400; el worker `9dd112d1-d002-471f-b123-4abe3012a1d5` respondió 503 porque descartaba el error del proveedor. Los tres planes locales consultados por la API de Mercado Pago estaban `active` con importe y moneda esperados. La causa exacta del 400 no se conserva en los logs anteriores. El cliente ahora recibe 422 con códigos seguros del proveedor solo en Test y el worker registra esos códigos sin datos de tarjeta ni correo. Se quitó `notification_url` del cuerpo de `/preapproval`, ya que no figura en la [referencia oficial de creación](https://www.mercadopago.com.ar/developers/es/reference/online-payments/subscriptions/create-preapproval/post); el webhook se registra en el panel.
